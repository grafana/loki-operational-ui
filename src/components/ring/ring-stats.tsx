...
export function RingStats({ nextRefresh, isPaused, onRefresh, instancesByState }: RingStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <span className="sr-only">{isPaused ? 'Auto-refresh paused' : 'Refresh ring data'}</span>
            </Button>
          </div>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{activeInstances}</div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium text-muted-foreground">Inactive Instances</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{inactiveInstances}</div>
        </CardContent>
      </Card>
    </div>
  );
