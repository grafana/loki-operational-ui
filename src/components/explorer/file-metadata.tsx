...
export function FileMetadataView({ metadata, filename, downloadUrl }: FileMetadataViewProps) {
  ...
  const totalCompressed = metadata.sections.reduce((sum, section) => sum + section.totalCompressedSize, 0);
  const totalUncompressed = metadata.sections.reduce((sum, section) => sum + section.totalUncompressedSize, 0);

  // Get stream and log counts from first column of each section
  const streamSection = metadata.sections.filter((s) => s.type === 'SECTION_TYPE_STREAMS');
  const logSection = metadata.sections.filter((s) => s.type === 'SECTION_TYPE_LOGS');
  const streamCount = streamSection?.reduce((sum, sec) => sum + (sec.columns[0].rows_count || 0), 0);
  const logCount = logSection?.reduce((sum, sec) => sum + (sec.columns[0].rows_count || 0), 0);

  return (
    <Card className="w-full">
      <FileHeader filename={filename} downloadUrl={downloadUrl} lastModified={metadata.lastModified} />
...
function HeadlineStats({ totalCompressed, totalUncompressed, sections, streamCount, logCount }: HeadlineStatsProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div className="rounded-lg bg-muted/50 p-6 shadow-sm">
        ...
        <CompressionRatio compressed={totalCompressed} uncompressed={totalUncompressed} showVisualization />
      ...
      <div className="rounded-lg bg-muted/50 p-6 shadow-sm">
        ...
        <div className="text-xs text-muted-foreground mt-2">{sections.map((s) => s.type).join(', ')}</div>
      </div>
      {streamCount && (
        <div className="rounded-lg bg-muted/50 p-6 shadow-sm">
          <div className="text-sm text-muted-foreground mb-2">Stream Count</div>
          <div className="font-medium text-lg">{streamCount.toLocaleString()}</div>
        </div>
      )}
    ...
    <div className="divide-y divide-border">
      {sections.map((section, sectionIndex) => (
        <Section
          ...
          onToggleColumn={(columnIndex) => onToggleColumn(sectionIndex, columnIndex)}
        />
      ))}
    </div>
  );
...
function Section({ section, sectionIndex, isExpanded, expandedColumns, onToggle, onToggleColumn }: SectionProps) {
  return (
    <div className="py-4">
      <button
        ...
        <div className="rounded-lg bg-muted/50 p-6 shadow-sm">
          ...
          <div className="space-y-2">
            <div className="text-sm">
              Spanning{' '}
              {Math.ceil(
                (new Date(section.maxTimestamp).getTime() - new Date(section.minTimestamp).getTime()) /
                  (1000 * 60 * 60) +
                  1
              )}{' '}
              hours
              <span className="text-sm text-muted-foreground">
                <span> from </span>
                <DateHover date={new Date(section.minTimestamp)} /> to{' '}
                <DateHover date={new Date(section.maxTimestamp)} />
              </span>
            </div>

            <div className="text-sm">Age within object</div>
...
function ColumnsList({ columns, sectionIndex, expandedColumns, onToggleColumn }: ColumnsListProps) {
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      <div className="rounded-lg bg-muted p-6">
        ...
        <div className="font-medium">
          <CompressionRatio compressed={column.compressed_size} uncompressed={column.uncompressed_size} />
        </div>
        <div className="text-xs text-muted-foreground mt-2">
      ...
      <div className="rounded-lg bg-muted p-6">
        ...
        <div className="font-medium text-lg">{column.rows_count.toLocaleString()}</div>
      </div>
      <div className="rounded-lg bg-muted p-6">
        <div className="text-sm text-muted-foreground mb-2">Values Count</div>
        <div className="font-medium text-lg">{column.values_count.toLocaleString()}</div>
      </div>
      <div className="rounded-lg bg-muted p-6">
        <div className="text-sm text-muted-foreground mb-2">Offset</div>
        <div className="font-medium text-lg">{formatBytes(column.metadata_offset)}</div>
      </div>
      {column.statistics && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {column.statistics.cardinality_count !== undefined && (
                <div>
                  ...
                  <div className="font-medium">{column.statistics.cardinality_count.toLocaleString()}</div>
                </div>
              )}
            </div>
          </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-secondary/50 border-b border-border">
              ...
              <th className="text-left p-4 font-medium text-muted-foreground">Rows</th>
              <th className="text-left p-4 font-medium text-muted-foreground">Values</th>
              <th className="text-left p-4 font-medium text-muted-foreground">Encoding</th>
              <th className="text-left p-4 font-medium text-muted-foreground">Compression</th>
            </tr>
          </thead>
          <tbody>
            {pages.map((page, pageIndex) => (
              <tr key={pageIndex} className="border-t border-border hover:bg-accent/50 transition-colors">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <CompressionRatio compressed={page.compressed_size} uncompressed={page.uncompressed_size} />
