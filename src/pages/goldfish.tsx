...
export default function GoldfishPage() {
  ...
  }, [timeRange.from, timeRange.to, getDefaultTimeRange]);
        ...
        <CardHeader>
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="flex items-center justify-end gap-4">
              <div className="text-sm text-muted-foreground">Showing queries from:</div>
              <TimeRangeSelector
                from={timeRange.from}
                to={timeRange.to}
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {uniqueTenants.length > 0 && (
                  <TenantFilterSelect value={selectedTenant} onChange={setSelectedTenant} tenants={uniqueTenants} />
                )}

                {/* User Filter - Combobox with dropdown */}
                <UserFilterCombobox value={selectedUser} onChange={setSelectedUser} suggestions={uniqueUsers} />
          ...
          <div className="space-y-4">
            {timeRangeError && (
              <Alert variant="destructive">
                ...
                <AlertDescription>{timeRangeError}</AlertDescription>
              </Alert>
            )}
            ...
            {isLoading && queries.length === 0 ? (
              ...
              <>
                <div className="text-center py-12 text-muted-foreground">
                  <p>
                    No queries found in current results
                    {selectedTenant && ` for tenant ${selectedTenant}`}
                    {selectedUser && ` for user ${selectedUser}`}
                  </p>
                  {hasMore && <p className="mt-2">Try loading more results to find matching queries</p>}
                </div>
                ...
                {!hasMore && queries.length > 0 && (
                  <div className="text-center py-6 text-muted-foreground text-sm">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span>No more results ({queries.length} total queries)</span>
                      <div className="h-px bg-border flex-1 max-w-xs" />
                    </div>
                  </div>
                )}
